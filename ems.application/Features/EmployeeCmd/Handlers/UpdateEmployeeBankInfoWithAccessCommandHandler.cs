...
{
    public class UpdateEmployeeBankInfoWithAccessCommandHandler : IRequestHandler<UpdateEmployeeBankDetailWithAccessCommand, ApiResponse<bool>>
    {
        private readonly IEmployeeRepository _employeeRepository;
        private readonly IUnitOfWork _unitOfWork;
        private readonly ILogger<UpdateEmployeeBankInfoWithAccessCommandHandler> _logger;
        private readonly IMapper _mapper;

        public UpdateEmployeeBankInfoWithAccessCommandHandler(
            IEmployeeRepository employeeRepository,
            IUnitOfWork unitOfWork,
            ILogger<UpdateEmployeeBankInfoWithAccessCommandHandler> logger,
            IMapper mapper)
        {
        ...
        public async Task<ApiResponse<bool>> Handle(UpdateEmployeeBankDetailWithAccessCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var employee = await _employeeRepository.GetEmployeeBankInfoByIdAsync(dto.EmployeeId);
                if (employee == null)
                    return ApiResponse<bool>.Fail("Employee not found.");

                var createDto = _mapper.Map<EmployeeBankEditableFieldsDTO>(employee);

                // 👇 Step 2: Generate FieldWithAccess version (for read-only info)
                var accessDto = EmployeeBankInfoMapperHelper.ConvertToAccessResponseDTO(createDto);

                // 👇 Step 3: Find property info (case-insensitive)
                var accessProp = typeof(GetEmployeeBankDetailWithAccessResponseDTO)
                ...
                var employeeProp = typeof(EmployeeBankDetail)
                    .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                    .FirstOrDefault(p => string.Equals(p.Name, dto.FieldName, StringComparison.OrdinalIgnoreCase));
