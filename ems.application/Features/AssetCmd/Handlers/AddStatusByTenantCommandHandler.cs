...
{
    public class AddStatusByTenantCommandHandler : IRequestHandler<AddStatusByTenantCommand, ApiResponse<List<AssetStatusResponseDTO>>>
    {
        ...
        public async Task<ApiResponse<List<AssetStatusResponseDTO>>> Handle(AddStatusByTenantCommand request, CancellationToken cancellationToken)
        {
            try
            {
             ...
             List <AssetStatus> assetsList = await _unitOfWork.AssetRepository.AddAssetStatusByTenantAsync(assetStatus);
              
                // Entity list to DTO list
                var resultDTOList = _mapper.Map<List<AssetStatusResponseDTO>>(assetsList);
